...
function guardarColores(){
    ...
    if(nombrePaleta==null || nombrePaleta==""){
        ...
        var listaBotonesColores=document.querySelectorAll(".circuloVacio");
        for (let i = 0; i < listaBotonesColores.length; i++) {
            arrayColores.push(listaBotonesColores[i].style.background);
        }
        var datos=[cadenaClave,arrayColores];
        datosGuardar.push(datos);
        var cadenaEnvio="guardado="+JSON.stringify(datosGuardar)
                ...
                `   <div class="dato">
                    ...
                    for (let i = 0; i < arrayColores.length; i++) {
                        if(arrayColores[i]==""){
                            ...
                            cajaColores.innerHTML+=`
                            <span class="circuloRelleno" style="background: ${arrayColores[i]}"></span>
                            `
                        }
                    }
        ...
        xhttp.open("POST","conexionColores.php");
        xhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        xhttp.send(cadenaEnvio); 
    }
                    ...
                    `   <div class="dato">
                        ...
                        for (let j = 0; j < datos[i][1].length; j++) {
                            ...
                            if(dato==null){
                                ...
                                cajaColores.innerHTML+=`
                                <span class="circuloRelleno" style="background: ${datos[i][1][j]}"></span>
                                `
                            }
                        }
...
function borrar(){
    ...
    for (let i = 0; i < listaBotonesColores.length; i++) {
        listaBotonesColores[i].addEventListener("click", (evento) => {
            ...
            var cadenaEnvio="borrar="+JSON.stringify(id)
            const xhttp=new XMLHttpRequest();
            xhttp.onload=function(){
                if(this.readyState==4 && this.status==200){
                    cargarLista();
